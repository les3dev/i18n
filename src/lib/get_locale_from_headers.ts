 ...
 * ```
 */
export function get_locale_from_headers<T extends string>(headers: Headers, i18n: {locales: readonly string[]; default_locale: T}): T {
    const header = headers.get('accept-language');
    if (!header) return i18n.default_locale;
